...
| v6.1 R4-P0 TRAIN-only gradient audit | Passed; routing fixed | — | — | — | — |
| v6.1 R4-P1 fresh repaired-lattice parent | Drop after screen; reproducible P2 parent | 0.8139 | 0.7122 | 0.8852 | 0.7638 |
| v6.1 R4-P2 matched focal control | Active; Relation E21 archived; E16 selected | 0.8138 | 0.7207 | 0.8831 | 0.7647 |
...
R3-P3 is closed as `DROP_AFTER_SCREEN`, and the R3 line is complete. Its corrected v2 transfer preserved the protected R1 metrics but passed only three of twelve gates, confirming that the R3-P2 ranking gain was representation- and calibration-entangled rather than transferable as standalone scorer tensors. Final R3-P3 A/B closure packages were independently re-materialized and checksum-verified. See [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), [the decision](docs/TurkTokenizer_v6_0_R3_P3_Decision.md), and [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md). R4-P0 then passed its zero-update TRAIN-only gradient-conflict audit; see the [R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md) and [decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md). R4-P1 is the precommitted fresh repaired-lattice reconstruction with no adapter, PCGrad, ranking loss, or checkpoint migration; see [the R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md). Syntax closed safely at E27 with E24 frozen as the selected parent, and Relation closed at E31 with E22 selected at `0.78921340`. Hard-Negative closed at H12 under the original patience rule. H12 produced TRAIN loss `0.0832`, gold-CALIB combined objective loss `3.78820134`, macro F1 `0.80876193`, minimum-family/OBJECT F1 `0.70588235`, `UAS=0.88377213`, `LAS=0.76395004`, `UPOS=0.92312876`, and selection score `0.78384843`. H03 remains selected at `0.78862166`; patience reached `9/9` and learning rate remained `0.00006`. The H10-H12 overfit counter stayed at `0/3`: H12's CALIB objective loss rose, but TRAIN loss also rose, so no loss-divergence signal fired. Both 26-file H12 closure archives and all active/durable mirrors passed independent verification. H13-H14 were not run because the authoritative patience stop closed the stage; H15 was not started. Sealed evaluation remains unopened.

The final R4-P1 screen used the unchanged selected H03 checkpoint and was reproduced exactly in two independent executions, with a third independent CPU recomputation. It yielded macro F1 `0.81389095`, minimum-family/OBJECT F1 `0.71215207`, `UAS=0.88520981`, and `LAS=0.76381526`. Macro gain over A1 was below the precommitted screen threshold and minimum-family F1 regressed, so the decision is `DROP_AFTER_SCREEN`. R4-P1 nevertheless supplies the reproducible fresh focal parent required for an R4-P2 interference-isolation precommit. Its two private 37-file final-closure packages were independently re-materialized and checksum-verified. The R4-P2 matched focal control is now precommitted and has passed its zero-step start gate. It begins Relation fresh from the verified R4-P1 Syntax E24 parent, reuses the unchanged focal trainer/objective, and disables adapters, PCGrad, and ranking loss. Its 16-file start boundary passed independent A/B re-materialization and all manifest checksums. Relation E21 is the current verified boundary; E16 remains selected at `0.79078781`. E21 did not improve; patience is `5/9`, LR remains `0.00003125`, and falling TRAIN with rising CALIB objective advances divergence to `1/3`. The completed retry is the sole canonical E21 boundary. Both E21 A/B packages passed independent checksum, byte-equality, checkpoint, and reconstructed-state verification. E22 is pending; see [the control progress log](docs/TurkTokenizer_v6_1_R4_P2_Control_Progress.md). Sealed evaluation remains unopened.
