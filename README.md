...
| v6.1 R4-P1 fresh repaired-lattice parent | Hard-Negative H09 complete; H03 selected | 0.8117 | 0.7164 | 0.8841 | 0.7657 |
...
R3-P3 is closed as `DROP_AFTER_SCREEN`, and the R3 line is complete. Its corrected v2 transfer preserved the protected R1 metrics but passed only three of twelve gates, confirming that the R3-P2 ranking gain was representation- and calibration-entangled rather than transferable as standalone scorer tensors. Final R3-P3 A/B closure packages were independently re-materialized and checksum-verified. See [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), [the decision](docs/TurkTokenizer_v6_0_R3_P3_Decision.md), and [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md). R4-P0 then passed its zero-update TRAIN-only gradient-conflict audit; see the [R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md) and [decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md). R4-P1 is the precommitted fresh repaired-lattice reconstruction with no adapter, PCGrad, ranking loss, or checkpoint migration; see [the R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md). Syntax closed safely at E27 with E24 frozen as the selected parent, and Relation closed at E31 with E22 selected at `0.78921340`. Hard-Negative H09 completed from the verified H08 boundary under the reduced learning rate with full source hard-negative penalty `0.250000`. It produced TRAIN loss `0.0814`, macro F1 `0.81167239`, minimum-family/OBJECT F1 `0.71643836`, `UAS=0.88408662`, `LAS=0.76565729`, `UPOS=0.92160122`, and selection score `0.78853128`. H09 remained approximately `0.00009038` below the locked H03 score, so H03 remains the selected Hard-Negative checkpoint at `0.78862166`; patience is `6/9` and learning rate remains `0.00006`. Both 24-file H09 archives were independently re-materialized and checksum-verified, and all state/cache/checkpoint mirrors matched byte-for-byte. The canonical boundary is H09 with H10 not started. A non-retroactive Hard-Negative overfit guard is precommitted from H10: the counter begins at `0/3`, H10 establishes a fresh TRAIN/gold-CALIB objective-loss baseline, and H04-H09 are not counted retrospectively. Starting with H11, only consecutive non-improving epochs with falling TRAIN loss and at least a 0.1% rise in gold-CALIB objective loss increment the counter; `3/3` closes the stage. The original `9/9` patience rule remains authoritative. Sealed evaluation remains unopened.
