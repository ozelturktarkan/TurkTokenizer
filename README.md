...
| v6.1 R4-P1 fresh repaired-lattice parent | Syntax E25 complete; E24 selected | — | — | 0.8823 | 0.7640 |
...
R3-P3 is closed as `DROP_AFTER_SCREEN`, and the R3 line is complete. Its corrected v2 transfer preserved the protected R1 metrics but passed only three of twelve gates, confirming that the R3-P2 ranking gain was representation- and calibration-entangled rather than transferable as standalone scorer tensors. Final R3-P3 A/B closure packages were independently re-materialized and checksum-verified. See [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), [the decision](docs/TurkTokenizer_v6_0_R3_P3_Decision.md), and [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md). R4-P0 then passed its zero-update TRAIN-only gradient-conflict audit; see the [R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md) and [decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md). R4-P1 is the precommitted fresh repaired-lattice reconstruction with no adapter, PCGrad, ranking loss, or checkpoint migration; see [the R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md). Syntax E09 through E25 completed without recurrence of the isolated E07 TRAIN-loss anomaly. Fresh Syntax E25 completed from the verified E24 boundary with TRAIN loss `0.2628`, CALIB syntax loss `1.7332`, `UAS=0.8823`, `LAS=0.7640`, `UPOS=0.9231`, and selection score `0.81541918`. E25 did not improve the locked score, so E24 remains the selected syntax checkpoint and patience advanced from `0/9` to `1/9`; the learning rate remains `0.000125`. TRAIN loss fell while CALIB syntax loss rose beyond the precommitted relative threshold, producing the first consecutive overfit signal (`1/3`); no safety stop fired and the learning rate was not reduced. Active and durable E25 resume-state mirrors matched byte-for-byte, while the selected E24 checkpoint remained unchanged and mirrored. Both E25 archives were independently re-materialized, checksum-verified, state-reconstructed, and byte-compared. E26 is authorized under learning rate `0.000125` and the unchanged one-epoch protocol with explicit overfit-streak monitoring; sealed evaluation remains unopened.
