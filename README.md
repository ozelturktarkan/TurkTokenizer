...
| v6.0 R3-P3 family-isolated ranking transfer | v1 invalidated; v2 graft precommitted | — | — | — | — |
...
R3-P3 remains a controlled follow-up, not a synthesis line. Its first masked-training design was invalidated after Relation E01 revealed that the selected Syntax E10 start boundary could not represent the completed R1 protected branches; E02 was not run, the diagnostic E01 values are not an R3-P3 result, and no promotion decision may use them. The corrected v2 experiment instead starts from the completed frozen R1 checkpoint and deterministically grafts exactly 123 direct-family tensors (2,222,643 parameters) from the closed R3-P2 ranking candidate. The other 216 tensors (27,083,444 parameters), including the complete `CASE_GOVERNOR` and syntax paths, remain bit-identical to R1. No optimizer step is taken. The parent reproduction, graft smoke, start gate, and duplicate precommit packages are verified; the single CALIB evaluation is pending under the unchanged twelve gates. See [the original precommit](docs/TurkTokenizer_v6_0_R3_P3_Precommit.md), [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), and [progress](docs/TurkTokenizer_v6_0_R3_P3_Progress.md).
