...
| v6.1 R4-P2 matched focal control | Active; Relation E03 archived | 0.7960 | 0.7065 | 0.8775 | 0.7594 |
...
The final R4-P1 screen used the unchanged selected H03 checkpoint and was reproduced exactly in two independent executions, with a third independent CPU recomputation. It yielded macro F1 `0.81389095`, minimum-family/OBJECT F1 `0.71215207`, `UAS=0.88520981`, and `LAS=0.76381526`. Macro gain over A1 was below the precommitted screen threshold and minimum-family F1 regressed, so the decision is `DROP_AFTER_SCREEN`. R4-P1 nevertheless supplies the reproducible fresh focal parent required for an R4-P2 interference-isolation precommit. Its two private 37-file final-closure packages were independently re-materialized and checksum-verified. The R4-P2 matched focal control is now precommitted and has passed its zero-step start gate. It begins Relation fresh from the verified R4-P1 Syntax E24 parent, reuses the unchanged focal trainer/objective, and disables adapters, PCGrad, and ranking loss. Its 16-file start boundary passed independent A/B re-materialization and all manifest checksums. Relation E03 is the current verified boundary with macro F1 `0.7960`, minimum-family/OBJECT F1 `0.7065`, `UAS=0.8775`, `LAS=0.7594`, and selection score `0.77578691`. The score improved, so the overfit counter remains `0/3`; E03 passed independent A/B boundary verification. E04 is pending; see [the control progress log](docs/TurkTokenizer_v6_1_R4_P2_Control_Progress.md). Sealed evaluation remains unopened.
