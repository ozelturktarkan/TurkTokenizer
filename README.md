...
| v6.1 R4-P1 fresh repaired-lattice parent | Syntax closed safely at E27; E24 frozen | — | — | 0.8819 | 0.7648 |
...
R3-P3 is closed as `DROP_AFTER_SCREEN`, and the R3 line is complete. Its corrected v2 transfer preserved the protected R1 metrics but passed only three of twelve gates, confirming that the R3-P2 ranking gain was representation- and calibration-entangled rather than transferable as standalone scorer tensors. Final R3-P3 A/B closure packages were independently re-materialized and checksum-verified. See [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), [the decision](docs/TurkTokenizer_v6_0_R3_P3_Decision.md), and [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md). R4-P0 then passed its zero-update TRAIN-only gradient-conflict audit; see the [R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md) and [decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md). R4-P1 is the precommitted fresh repaired-lattice reconstruction with no adapter, PCGrad, ranking loss, or checkpoint migration; see [the R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md). Syntax E09 through E27 completed without recurrence of the isolated E07 TRAIN-loss anomaly. Fresh Syntax E27 completed from the verified E26 boundary with TRAIN loss `0.2003`, CALIB syntax loss `1.8357`, `UAS=0.8819`, `LAS=0.7648`, `UPOS=0.9220`, and selection score `0.81561686`. E27 did not improve the locked score, so E24 remains selected; patience advanced from `2/9` to `3/9` and the learning rate remained `0.000125`. TRAIN loss fell while CALIB syntax loss rose for a third consecutive qualifying epoch, so the precommitted overfit guard reached `3/3` and stopped Syntax safely. The selected E24 checkpoint was copied to the frozen Syntax checkpoint, and the safety and completion markers were persisted. Active and durable E27 closure artifacts matched byte-for-byte. Both 21-file E27 closure archives were independently re-materialized, checksum-verified, state-reconstructed, and byte-compared. Syntax is closed at E27; Syntax E28 is not authorized, and any later phase remains conditional on a separate continuation decision. Sealed evaluation remains unopened.
