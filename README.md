...
| v6.1 R4-P1 fresh repaired-lattice parent | Syntax E07 complete; anomaly audited, training active | — | — | 0.8573 | 0.7304 |
...
R3-P3 is closed as `DROP_AFTER_SCREEN`, and the R3 line is complete. Its original masked-training design was invalidated after Relation E01 because the Syntax E10 start boundary could not represent the completed R1 protected branches; E02 did not run and the diagnostic E01 values were never treated as a result. The corrected v2 experiment grafted exactly 123 direct-family tensors (2,222,643 parameters) from R3-P2 onto the completed frozen R1 checkpoint with zero optimizer steps. It preserved R1 `CASE_GOVERNOR`, UAS, and LAS exactly, but yielded macro F1 `0.7795`, minimum-family/OBJECT F1 `0.7139`, `UAS=0.8831`, and `LAS=0.7643`, passing only those three protected-metric gates out of twelve. POSS_HEAD and PARTICIPLE_HEAD also fell sharply, so the R3-P2 ranking gain is representation- and calibration-entangled rather than localized in the donor scorer modules. Final A/B closure packages, including the audit checkpoint, were re-materialized and checksum-verified. See [the corrected v2 precommit](docs/TurkTokenizer_v6_0_R3_P3_v2_Precommit.md), [the decision](docs/TurkTokenizer_v6_0_R3_P3_Decision.md), and [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md). R4-P0 passed as a zero-update, TRAIN-only gradient-conflict audit. All 12 batches completed with unchanged model state and no CALIB access. The fixed P2 routing uses PCGrad on the shared Transformer, relation bridge, and syntax bridge; see [the R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md) and [decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md). R4-P1 is precommitted as a fresh repaired-lattice reconstruction with no adapter, PCGrad, ranking loss, or checkpoint migration; see [the R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md). Fresh Syntax E07 completed with CALIB syntax loss `1.3756`, `UAS=0.8573`, `LAS=0.7304`, `UPOS=0.9055`, and selection score `0.78600503`. TRAIN loss `248.4047` was traced to one deterministic batch: its syntax-head loss spiked while every other batch remained below `1.72`. Gradient clipping preserved a finite checkpoint, the locked selection score improved, and a 4,096-sample dropout stress test of the selected E07 checkpoint produced no recurrence. E07 therefore remains canonical and E08 is authorized under the unchanged protocol with explicit recurrence monitoring. Patience and overfit streak remain `0/9` and `0/3`; learning rate remains `0.0005`. E07 state and decision archive pairs were independently re-materialized and checksum-verified; sealed evaluation remains unopened.
