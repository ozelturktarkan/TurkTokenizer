...
def main():
    require(not (LOCAL/'reachable-output-audit.json').exists(),'P4_REFUSE_OVERWRITE_REACHABILITY')
    comp=read(LOCAL/'comparison.json');name=comp['retained'];rt=Tokenizer(**comp['configs'][name]);splits={}
    for split in ('dev','calib'):
        info=comp['arms'][name][split];path=Path(info['output_path'])
        require(digest(path)==info['artifact_sha256']['outputs.jsonl.gz'],'P4_REACHABILITY_OUTPUT_CHANGED')
        counts=collections.Counter()
        for ref,rec in zip(records(ROOT/f'training/a1-large-v1/{split}.jsonl'),records(path),strict=True):
            require(ref['id']==rec['id'],'P4_REACHABILITY_REFERENCE_MISMATCH')
            ...
            for u in ref['units']:
                ...
                i,t=pair;aa=t['analysis']['analyses'];scores=rt.export_head.token(out['tokens'],i)
                p=t.get('context_decision',{}).get('preferred_analysis');chosen=t['lexical_decision']['preferred_view']
                if p:require(scores[p['analysis_id']]['view']==chosen,'P4_HEAD_SELECTION_CHANGED')
        ...
        require(counts['correct_selected_view']==info['views']['predicted_view_lemma_pos'],'P4_REACHABILITY_COUNT_MISMATCH')
        require(counts['inventory_view_oracle']==info['views']['oracle_view_lemma_pos'],'P4_REACHABILITY_ORACLE_MISMATCH')
        require(sum(counts[k] for k in ('correct_selected_view','wrong_but_reachable_by_native_reranking','view_exists_but_fixed_head_blocks_every_native_choice','no_matching_view_or_unresolved'))==counts['words'],'P4_REACHABILITY_PARTITION')
