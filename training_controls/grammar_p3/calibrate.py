...
def main():
    require(not (DEST/'A2').exists(),'P3_REFUSE_OVERWRITE_A2')
    (DEST/'A2').mkdir();comparison=read(LOCAL/'comparison.json');cfg=read(HERE/'protocol.json')['A2']
    require(comparison['status']=='EXPERIMENT_COMPLETE','P3_EVALUATION_INCOMPLETE')
    name=comparison['retained_configuration'];results={}
    for key in dict.fromkeys(['C0_P2',name]):
        result=comparison['arms'][key]['calib'];folder=Path(result['output_path']).parent
        require(all(digest(folder/n)==h for n,h in result['artifact_sha256'].items()),'P3_CALIB_ARTIFACT_CHANGED')
        rows=list(records(folder/'native-words.jsonl.gz'))
        grid,best=choose(rows,cfg['threshold_grid'],cfg['target_empirical_precision_pct'])
        ...
        rt=Tokenizer(key,a2=thresholds);codec=Codec(key,a2=thresholds);codec.native=rt
        ...
        for ref,rec in zip(refs,records(folder/'outputs.jsonl.gz'),strict=True):
            require(ref['id']==rec['id'],'P3_CALIB_ALIGNMENT')
            ...
            if thresholds:
                ...
                require([t.get('context_decision') for t in actual['tokens']]==[t.get('context_decision') for t in expected['tokens']],'P3_LIVE_CALIBRATION_MISMATCH')
                require([t.get('decision_layers') for t in actual['tokens']]==[t.get('decision_layers') for t in expected['tokens']],'P3_LIVE_LAYER_MISMATCH')
                require(codec.decode(codec.encode_from_analysis(actual)['input_ids'])==ref['text'],'P3_A2_BPE_ROUNDTRIP')
                roundtrips+=1
        require(counts==[a2_summary(rows,t)['accepted'] for t in checks],'P3_CALIB_ACCEPTANCE_COUNT')
        ...
        print(json.dumps({'stage':'P3_A2_COMPLETE','configuration':key,'status':results[key]['status'],'selected':best}),flush=True)
    winner=results[name]
    selection={'status':winner['status'],'configuration':name,'A2_config':winner['selected']['thresholds'] if winner['selected'] else None,
