...
Relation closed at E31 under the precommitted patience rule with E22 retained as the selected Relation checkpoint at `0.78921340`. Hard-Negative H09 completed from the verified H08 boundary under the reduced learning rate, using the unchanged deterministic TRAIN-only cache (82 CASE_GOVERNOR, 432 OBJECT, 290 PARTICIPLE_HEAD, and 426 POSS_HEAD examples) and full source hard-negative penalty `0.250000`. H09 produced TRAIN loss `0.0814`, macro F1 `0.81167239`, minimum-family/OBJECT F1 `0.71643836`, `POSS_HEAD=0.80478088`, `OBJECT=0.71643836`, `PARTICIPLE_HEAD=0.84007707`, `CASE_GOVERNOR=0.88539326`, `UAS=0.88408662`, `LAS=0.76565729`, `UPOS=0.92160122`, and selection score `0.78853128`. H09 remained approximately `0.00009038` below the locked H03 Hard-Negative score of `0.78862166`, so H03 remains selected; patience is `6/9` and the learning rate remains `0.00006`. Active and durable state, cache, and checkpoint mirrors match byte-for-byte, while the locked Relation E22 and Syntax E24 parents remain unchanged. Both 24-file H09 archives were independently re-materialized, checksum-verified, state-reconstructed, and byte-compared. The canonical boundary is H09 with H10 not started. Before H10, a non-retroactive Hard-Negative overfit guard was precommitted at `0/3`. H10 establishes the first TRAIN/gold-CALIB objective-loss baseline and cannot increment the counter; H04-H09 are not backfilled. From H11, three consecutive non-improving epochs in which TRAIN loss falls while the gold-CALIB objective loss rises by at least 0.1% close the stage and freeze the selected best checkpoint. The existing `9/9` patience rule remains authoritative and may close the stage earlier. No R2-P9 E28, R1, or R3 checkpoint is migrated. The suffix/allomorph registry remains outside R4. `INTERNAL_VAL`, external holdouts, and official TEST remain unopened. See [the R4 plan](docs/TurkTokenizer_v6_1_R4_Plan.md), [R4-P0 precommit](docs/TurkTokenizer_v6_1_R4_P0_Precommit.md), [R4-P0 decision](docs/TurkTokenizer_v6_1_R4_P0_Decision.md), [R4-P1 precommit](docs/TurkTokenizer_v6_1_R4_P1_Precommit.md), and [R4-P1 progress](docs/TurkTokenizer_v6_1_R4_P1_Progress.md).
